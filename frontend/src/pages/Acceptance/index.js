...
const Acceptance = () => {
    ...
    console.log("header in jo file chahiye", { header })
    const [acceptData, setAcceptData] = useState([])

    useEffect(() => {
        const employeeId = localStorage.getItem("employee_id");

        axiosInstance.get(`distribution/getPendingAcceptance/${employeeId}`, header).then(data => {
            console.log({ data })
                    ...
                    <table >
                        <thead>
                            <tr >
                                ...
                                <th className="table-row-data">received from</th>
                            ...
                            <tbody >
                                {acceptData.map((row, i) => (
                                    <tr key={i}>
                                        ...
                                        <td className="table-row-data">{row.senderEmployeeId.employee_id}</td>
                                        <td className="table-row-data">{row.senderEmployeeId.employee_name}</td>
                                        <td className="table-row-data extra-width">{row.senderEmployeeId.email}</td>
                                        <td className='buttons table-row-data'>
